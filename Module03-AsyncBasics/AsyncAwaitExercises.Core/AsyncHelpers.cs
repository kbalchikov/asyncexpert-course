...
namespace AsyncAwaitExercises.Core
{
    public class AsyncHelpers
    {
        public static async Task<string> GetStringWithRetries(HttpClient client, string url, int maxTries = 3, CancellationToken token = default)
        {
            ...
            {
                string body;
                (exc, body) = await TrySendRequest(client, url, token);
        ...
        public static async Task<(Exception, string)> TrySendRequest(HttpClient client, string url, CancellationToken token)
        {
            try
            {
